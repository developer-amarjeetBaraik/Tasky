import { cn } from "@/lib/utils"
import { Button } from "@/components/ui/button"
import { Card, CardContent } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { NavLink, useNavigate } from "react-router-dom"
import AppLogo from "./ui/AppLogo"
import { useRef } from "react"
import { useUserAuth } from "@/hooks/useUserAuth"
import { useTheme } from "./ui/theme-provider"
import { toast, Toaster } from "sonner"
import type { CustomError } from "@/types"

export function SignupForm({
  className,
  ...props
}: React.ComponentProps<"div">) {
  const nameRef = useRef<HTMLInputElement | null>(null)
  const emailRef = useRef<HTMLInputElement | null>(null)
  const passwordRef = useRef<HTMLInputElement | null>(null)
  const { signup } = useUserAuth()
  const { theme } = useTheme()
  const navigate = useNavigate()

  // Signup form submit handler
  const handleFormSubmit = (event: React.FormEvent<HTMLFormElement>): void => {
    event.preventDefault();
    if (!nameRef.current?.value) {
      toast.error("Please enter name.")
      return
    }
    if (nameRef.current?.value?.length < 4 || nameRef.current?.value?.length > 20) {
      toast.error("Name length must be between 4 to 20 characters.")
      return
    }
    if (!emailRef.current?.value) {
      toast.error("Please enter email.")
      return
    }
    if (emailRef.current?.value?.length < 11 || emailRef.current?.value?.length > 35) {
      toast.error("Email length must be between 11 to 35 characters.")
      return
    }
    if (!passwordRef.current?.value) {
      toast.error("Please enter password.")
      return
    }
    if (passwordRef.current?.value?.length < 5 || passwordRef.current?.value?.length > 15) {
      toast.error("Password length must be between 5 to 15 characters.")
      return
    }
    signup(nameRef.current?.value, emailRef.current?.value, passwordRef.current?.value,
      (error, success) => {
        console.log({ error, success })
        if (error) {
          toast.error((error as CustomError)?.message)
          return
        }
        if (success) {
          type CustomSuccess = {
            message: string,
            redirect: string,
          };
          navigate((success as CustomSuccess)?.redirect)
        }
      }
    )
  }

  return (
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <Toaster richColors position='top-center' theme={theme} duration={4000} />
      <Card className="overflow-hidden p-0">
        <CardContent className="grid p-0 md:grid-cols-2">
          <form onSubmit={handleFormSubmit} className="p-6 md:p-8">
            <div className="flex flex-col gap-6">
              <div className="flex flex-col items-center text-center">
                <h1 className="text-2xl font-bold">Welcome</h1>
              </div>
              <div className="grid gap-3">
                <Label htmlFor="name">Name</Label>
                <Input
                  id="name"
                  ref={nameRef}
                  type="text"
                  placeholder="Jhon"
                  required
                />
              </div>
              <div className="grid gap-3">
                <Label htmlFor="email">Email</Label>
                <Input
                  id="email"
                  ref={emailRef}
                  type="email"
                  placeholder="m@example.com"
                  required
                />
              </div>
              <div className="grid gap-3">
                <div className="flex items-center gap-3.5">
                  <Label htmlFor="password">Password</Label>
                </div>
                <Input id="password" ref={passwordRef} type="password" required />
              </div>
              <Button type="submit" className="w-full cursor-pointer">
                Signup
              </Button>
              <div className="after:border-border relative text-center text-sm after:absolute after:inset-0 after:top-1/2 after:z-0 after:flex after:items-center after:border-t">
                <span className="bg-card text-muted-foreground relative z-10 px-2">
                  Or continue with
                </span>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <Button variant="outline" type="button" className="w-full">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                    <path
                      d="M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z"
                      fill="currentColor"
                    />
                  </svg>
                  <span className="sr-only">Login with Google</span>
                </Button>
                <Button variant="outline" type="button" className="w-full">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" fill="currentColor" /></svg>
                  <span className="sr-only">Login with GitHub</span>
                </Button>
              </div>
              <div className="text-center text-sm">
                Already have an account?{" "}
                <NavLink to="/auth/login" className="underline underline-offset-4">
                  Login
                </NavLink>
              </div>
            </div>
          </form>
          <div className="bg-muted flex justify-center items-center gap-2.5">
            <AppLogo /> <h3 className="font-vibur text-black text-4xl dark:text-white">| Signup</h3>
          </div>
        </CardContent>
      </Card>
      <div className="text-muted-foreground *:[a]:hover:text-primary text-center text-xs text-balance *:[a]:underline *:[a]:underline-offset-4">
        By clicking continue, you agree to our <a href="#">Terms of Service</a>{" "}
        and <a href="#">Privacy Policy</a>.
      </div>
    </div>
  )
}
